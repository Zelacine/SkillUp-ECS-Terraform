...
aws_account_id=$(aws sts get-caller-identity --query Account --profile skillup | sed 's/\"//g')
...
# Log into AWS ECR
aws --profile skillup ecr get-login-password  | docker login --username AWS --password-stdin $repo
